...
def load_movies() -> list[dict]:
    ...
    Returns a list of dicts with keys: id, title, contextualized_overview.
    """
    ...
    rows = conn.execute(
        """
        SELECT id, title, release_year, contextualized_overview
        FROM   movies
    ...
    return [
        {
            ...
            "contextualized_overview": (contextualized_overview or "").strip(),
        }
        for movie_id, title, release_year, contextualized_overview in rows
    ]
...
def build_multi_chunks(movies: list[dict], chunk_size: int = CHUNK_SIZE) -> list[dict]:
    ...
    for start in range(0, total, chunk_size):
        ...
        divider = "\n" + "-" * 60 + "\n"
        merged_doc = divider.join(m["contextualized_overview"] for m in batch)
